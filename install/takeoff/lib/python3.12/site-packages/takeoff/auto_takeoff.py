...
Return-to-Launch (RTL) service
-------------------------------
Exposes a std_srvs/Trigger service, 'return_to_launch'. The Explorer node
(client) subscribes to /mavros/battery and calls this service once the
reported level drops below its threshold. On request, the mission stage
machine is frozen and the drone is sent to (0, 0, current_altitude) —
lateral correction only, altitude held at whatever it was when requested.
"""
...
class TakeoffPIDLand(Node):
    ...
    def __init__(self):
        ...
        # Return-to-launch (low-battery) state
        self.altitude   = 0.0;     # altitude held during RTL, set on request
        self.rtl_active = False   # set True once Explorer calls the service
        ...
        # Service for the Explorer node — called when battery < threshold
        self.rtl_srv = self.create_service(
            Trigger, 'return_to_launch', self.handle_rtl_request)
        ...
        self.get_logger().info(
            'Return-to-launch service ready on /return_to_launch')
    ...
    # ================================================================== #
    # RETURN-TO-LAUNCH SERVICE  (low battery)                             #
    # ================================================================== #

    def handle_rtl_request(self, request, response):
        """
        Called by the Explorer node once /mavros/battery drops below its
        threshold (0.3). Freezes the mission stage machine and points the
        drone at (0, 0, current_altitude) — lateral correction only, the
        altitude held is whatever the drone was at when this fired.
        """
        self.altitude = self.z_pos     # hold current altitude
        self.sp_x = 0.0
        self.sp_y = 0.0
        self.sp_z = self.altitude

        already = self.rtl_active
        self.rtl_active = True

        self.get_logger().warn(
            f'[RTL] Low-battery return requested by Explorer — '
            f'heading to (0.0, 0.0, {self.altitude:.2f})'
            + (' [already in progress]' if already else ''))

        response.success = True
        response.message = f'Returning to (0, 0, {self.altitude:.2f})'
        return response
    ...
    def image_callback(self, msg):
        if self.rtl_active or self.stage != 5:
            return
    ...
    def control_loop(self):
        ...
        # Low-battery RTL overrides the normal mission stage machine. The
        # fixed (0,0,altitude) setpoint set in handle_rtl_request is left
        # untouched and just keeps being republished by _publish_setpoint.
        if self.rtl_active:
            return
